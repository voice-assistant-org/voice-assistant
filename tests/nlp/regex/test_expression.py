...
@pytest.mark.parametrize(
    ...
    [("weather in <location>", "what's the weather in London mate", {"location": "London"},),],
)
...
def test_expression_match_finds_last_endity_end(expression, text, expected_last_entity_end):
    expr_match = NLPregexExpression(expression, entities=None).match(text)
    assert expr_match
...
def test_interpret_and_operator(expression, expected):
    assert (
        NLPregexExpression(expression, entities=None)._interpret_and_operator(expression)
        == expected
    )
